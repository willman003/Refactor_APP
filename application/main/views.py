...
def main_before_request():
    try:
        if not session.get('token'):
            return redirect(url_for('auth_token.login_for_token'))
    except ValueError:
        return redirect(url_for('auth_token.login_for_token'))

@main.route('/',methods=['GET','POST'])
def index():
    list_product = api_get_list_products(session['token'])
    dict_category = {}
    for item in api_get_list_category(session['token']):
...
def product_detail(id):
    form = Form_mua_hang()
    product = api_get_product(session['token'],id)
    list_product = api_get_list_products(session['token'])
    dict_category = {}
    for item in api_get_list_category(session['token']):
        dict_category[item['category_id']] = item['category_name']
